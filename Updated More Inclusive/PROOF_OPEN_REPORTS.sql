-- Query: rows behind OPEN_REPORTS for shoplifting incidents (NIBRS 23C)
-- Purpose: data-vetting extract for open reports
...
SELECT
    ...
    LISTAGG(DISTINCT ri.place_code, ', ') WITHIN GROUP (ORDER BY ri.place_code) AS place_codes,
    LISTAGG(DISTINCT ri.place_desc, ' | ') WITHIN GROUP (ORDER BY ri.place_desc) AS place_descs,
